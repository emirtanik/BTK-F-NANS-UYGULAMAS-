...
public class ChatService {
    ...
    public ChatResponse chat(Long userId, String userMessage) {
        ...
        String technicalInfo = buildTechnicalContext(userMessage);
    ...
    private String buildPortfolioContext(Long userId) {
        List<PortfolioItem> items = portfolioItemRepository.findAllByUserId(userId);
        if (items.isEmpty()) {
            return "Kullanicinin henuz portfoyu yok.";
        }

        StringBuilder sb = new StringBuilder();
        sb.append("KULLANICININ PORTFOY DURUMU:\n");
        ...
        for (PortfolioItem item : items) {
            ...
            BigDecimal invested = buyPrice.multiply(amount);
            ...
            sb.append(String.format(
                    "- %s: %s adet, alis %s TL, guncel %s TL, deger %s TL, kar/zarar %s TL (%%%.2f)%n",
                    item.getAssetSymbol(), amount, buyPrice,
                    currentPrice != null ? currentPrice : "bilinmiyor",
                    currentValue, pnl, pnlPercent.doubleValue()
            ));
        }
        ...
        sb.append(String.format("%nTOPLAM: yatirilan %s TL, guncel deger %s TL, net kar/zarar %s TL (%%%.2f)%n",
                totalInvested, totalCurrentValue, totalPnl, totalPercent.doubleValue()));

        long farkliVarliklar = items.stream().map(PortfolioItem::getAssetSymbol).distinct().count();
        sb.append(String.format("%nCESITLENDIRME: %d farkli varlik turu var.%n", farkliVarliklar));

        BigDecimal btc = marketDataService.getPriceTry("BTC");
        BigDecimal usd = marketDataService.getPriceTry("USD");
        BigDecimal altin = marketDataService.getPriceTry("GRAM_ALTIN");
        if (btc != null && usd != null && altin != null) {
            sb.append(String.format("%nGUNCEL PIYASA: BTC %s TL, USD/TRY %s, Gram Altin %s TL%n",
                    btc, usd, altin));
        }
    ...
    /**
     * Kullanici mesajinda gecen kripto sembolleri icin teknik analiz cikariyor,
     * Gemini'ye ekstra context olarak verecegiz.
     */
    private String buildTechnicalContext(String userMessage) {
        String upper = userMessage.toUpperCase();
        StringBuilder sb = new StringBuilder();

        String[] cryptoSymbols = {"BTC", "ETH", "BNB", "SOL", "XRP", "ADA"};
        for (String sym : cryptoSymbols) {
            if (upper.contains(sym) || upper.contains(getFullName(sym))) {
                try {
                    var analysis = technicalAnalysisService.analyze(sym);
                    sb.append(String.format(
                        "TEKNIK ANALIZ - %s:%n" +
                        "- Guncel fiyat: %s TL%n" +
                        "- RSI: %s (%s)%n" +
                        ...
                        analysis.rsi14() != null ? analysis.rsi14() : "veri yok",
                        analysis.rsiSignal(),
                        analysis.change7d() != null ? analysis.change7d().doubleValue() : 0.0,
                        analysis.change30d() != null ? analysis.change30d().doubleValue() : 0.0,
                        analysis.volatility() != null ? analysis.volatility().doubleValue() : 0.0,
